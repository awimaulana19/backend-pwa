<section id="registrasi">
    <div class="row">
        <div class="col-12">
            ...
            <p class="subtitle">Pemeriksaan Awal Kehamilan</p>
        </div>
    </div>

    <div class="row mt-4">
        <div class="col-12">
            <form id="form_pemeriksaanawal">
                @csrf
                <input type="text" name="antrian_sekarang" value="{{ $antrian_sekarang }}" hidden>
                <input type="text" name="user_id" value="{{ auth()->user()->id }}" hidden>
                <input type="text" name="periksa" value="PemeriksaanAwal" hidden>
                <div class="mb-3">
...
<script>
    $('#form_pemeriksaanawal').submit(function(event) {
        event.preventDefault();

        $.ajax({
            url: '{{ route('pemeriksaanawal_store') }}', // Ganti dengan URL yang sesuai untuk menyimpan data ke dalam database
            type: 'POST',
            data: $(this).serialize(),
